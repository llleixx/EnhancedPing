...
$project = Join-Path $projectRoot 'src\EnhancedPing\EnhancedPing.csproj'
$outputDir = Join-Path $projectRoot "src\EnhancedPing\bin\$Configuration"
...
if ([string]::IsNullOrWhiteSpace($Version) -or
    ...
    if ($LASTEXITCODE -ne 0) { throw 'Could not read package properties from EnhancedPing.csproj.' }
...
if (-not $SkipBuild) {
    ...
    if ($LASTEXITCODE -ne 0) { throw 'EnhancedPing build failed.' }
}

$pluginAssembly = Join-Path $outputDir 'EnhancedPing.dll'
